...
class PetitionClassifier:
    ...
    def __init__(self):
        ...
        # Load training data from file (1000+ examples)
    ...
    def _load_training_data(self):
        ...
        data_file = os.path.join(base_path, 'training_data.json')
        ...
        if os.path.exists(data_file):
            ...
            for item in data_list:
                if item['category'] in training_data:
                    training_data[item['category']].append(item['text'])
            
            print(f"✅ Loaded {len(data_list)} training examples from file")
            return training_data
        else:
            # Fallback to original small dataset
            print("⚠️  Using fallback training data (64 examples)")
            return self._get_fallback_data()
    
    def _get_fallback_data(self):
        """Original small training dataset as fallback"""
        return {
            "Education": [
