...
@petitions.route("/ocr", methods=["POST"])
@jwt_required()
def extract_text_from_image():
    ...
    try:
        ...
        if not OCRService.is_tesseract_installed():
            return jsonify({
                "error": "Tesseract OCR is not installed on the server. Please contact the administrator."
            }), 500
        ...
        result = OCRService.extract_text_from_image(file)
